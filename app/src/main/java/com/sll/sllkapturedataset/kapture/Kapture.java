...
public enum Kapture {
    ...
    ,RECORD_WIFI(7, "records_wifi","# timestamp, device_id, BSSID, frequency, RSSI, SSID, scan_time_start, scan_time_end", true)
    ,RECORD_BLE(8, "records_bluetooth","# timestamp, device_id, address, RSSI, name", true)
    ,RECORD_ACCEL(9, "records_accelerometer","# timestamp, device_id, x_acc, y_acc, z_acc", true)
    ,RECORD_GYRO(10, "records_gyroscope","# timestamp, device_id, x_speed, y_speed, z_speed", true)
    ,RECORD_MAG(11, "records_magnetic","# timestamp, device_id, x_strength, y_strength, z_strength", true)

   /**
     *sll custom part
     */
    , POSITION_QUERY_MAP(12, "gps_query_map","# timestamp, latitude, longitude", false)
    ,SESSION(13, "session", "# timestamp, latitude, longitude, heading", false)
    ;
